...
const Header = () => {
  ...
  const { cart, addToCart, removeFromCart, setIsCartOpen, isCartOpen } =
    useOrder();
  ...
  const placeOrder = async () => {
    ...
    const orderData = {
      orderId,
      email: localStorage.getItem("userEmail") || "guest@test.com",
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/swiggy/myorders", {
        method: "POST",
